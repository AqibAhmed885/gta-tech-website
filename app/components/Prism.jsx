...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const parseColor = (c) => {
      ...
      if (typeof c === 'string') {
        ...
        if (/^[0-9a-fA-F]{6}$/.test(hx)) {
          return [parseInt(hx.slice(0, 2), 16) / 255, parseInt(hx.slice(2, 4), 16) / 255, parseInt(hx.slice(4, 6), 16) / 255];
        }
        // rgb or "r,g,b"
        const m = s.match(/(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})/);
        if (m) return [parseInt(m[1], 10) / 255, parseInt(m[2], 10) / 255, parseInt(m[3], 10) / 255];
      }
      return [0.3725, 0.686, 0.866];
    };

    const tintRgb = parseColor(tintColor);
    ...
    const fragment = /* glsl */ `
      ...
      uniform float uTintStrength; // 0.0..1.0
      ...
      void main(){
        ...
        // blend with a tint color to allow easy site theming
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uTimeScale: { value: TS },
        uTintColor: { value: new Float32Array(tintRgb) },
